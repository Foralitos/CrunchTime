        ...
        private void DrawTransformControls()
{
    ...
    {
        Undo.RecordObject(door, "Save Open State"); // Enable undo
    ...
    {
        Undo.RecordObject(door, "Save Closed State"); // Enable undo
        ...
        private void SaveCurrentTransformStates(bool isOpen)
{
    ...
    // 🔥 THIS IS CRITICAL - Apply the changes!
    serializedDoor.ApplyModifiedProperties();
    EditorUtility.SetDirty(door); // Mark the object as modified
    
    Debug.Log($"Saved {(isOpen ? "Open" : "Closed")} State - Pos: {door.transform.localPosition}, Rot: {door.transform.localEulerAngles}");
}
